...
import { sonameFromDynamic } from "./native-metadata.mjs";
...
const families = [
  ...
  {
    source: "glib2.0",
    version: "2.88.3-0devtunnel1",
    packages: ["libglib2.0-0t64"],
  },
  {
    source: "p11-kit",
    version: "0.26.5-0devtunnel1",
    packages: ["libp11-kit0", "p11-kit", "p11-kit-modules"],
  },
];
function files(dir) {
...
function symbols(file) {
  return new Set(
    run("nm", ["-D", "--defined-only", "--format=posix", file])
      .split("\n")
      .filter(Boolean)
...
for (const family of families)
  for (const name of family.packages) {
    ...
    const download = `/native/download-${name}`;
    fs.mkdirSync(download);
    run("apt-get", ["download", `${name}=${originalVersion}`], {
      cwd: download,
    });
    const debs = fs.readdirSync(download).filter((x) => x.endsWith(".deb"));
    assert.equal(debs.length, 1);
    run("dpkg-deb", ["--raw-extract", path.join(download, debs[0]), dir]);
    const replaced = [];
    for (const old of files(dir).filter(
      (f) => !f.includes("/DEBIAN/") && elf(f),
    )) {
      const relative = path.relative(dir, old);
      const soname = sonameFromDynamic(run("readelf", ["-d", old]));
      let next,
        installed = relative;
      if (soname) {
        ...
        const rebuiltSoname = sonameFromDynamic(run("readelf", ["-d", next]));
        assert.equal(
          rebuiltSoname,
          soname,
          `ABI SONAME changed for ${relative}`,
        );
        const before = symbols(old),
          after = symbols(next);
        assert.deepEqual(
          [...before].filter((s) => !after.has(s)),
          [],
          `ABI symbols removed from ${relative}`,
        );
        ...
        // Public shared objects are readable, never writable, by group/others.
        // This package tree contains no credentials or runtime session files.
        ...
        if (!fs.existsSync(next)) {
          const matches = staged.filter(
            (f) => path.basename(f) === path.basename(old) && elf(f),
          );
          assert.equal(
            matches.length,
            1,
            `Ambiguous/missing executable ${relative}`,
          );
          next = matches[0];
        }
      ...
      replaced.push({
        file: relative,
        installed,
        rebuilt: path.relative(stage, next),
    ...
    metadata = metadata
      .replace(/^Version:.*$/m, `Version: ${family.version}`)
      .replace(/^Source:.*$/m, `Source: ${family.source} (${family.version})`)
      .replaceAll(`= ${originalVersion}`, `= ${family.version}`);
    if (!/^Source:/m.test(metadata))
      metadata += `Source: ${family.source} (${family.version})\n`;
    ...
    const subst = run(
      "dpkg-shlibdeps",
      ["-O", `-x${name}`, ...elfs.map((f) => `-e${f}`)],
      { cwd: "/native/control" },
    );
    const deps = /^shlibs:Depends=(.*)$/m.exec(subst)?.[1];
    assert.ok(deps, `Missing dependencies for ${name}`);
    metadata = metadata.replace(
      /^Depends: (.*)$/m,
      (_, old) => `Depends: ${old}, ${deps}`,
    );
    ...
    const doc = path.join(dir, "usr/share/doc/devtunnel-native");
    fs.mkdirSync(doc, { recursive: true });
    fs.writeFileSync(
      path.join(doc, `${name}.json`),
      JSON.stringify(provenance, null, 2) + "\n",
    );
    const payload = files(dir).filter(
      (f) => !f.includes("/DEBIAN/") && !fs.lstatSync(f).isSymbolicLink(),
    );
    const installedSize = Math.ceil(
      payload.reduce((n, f) => n + fs.statSync(f).size, 0) / 1024,
    );
    fs.writeFileSync(
      control,
      metadata.replace(
    ...
    // Debian's md5sums control file is a required-format inventory checksum,
    // NOT an authenticity/security check. Sources and ELF provenance use SHA-256.
    // Substituting SHA-256 here would corrupt the dpkg checksum file format.
    fs.writeFileSync(
      path.join(dir, "DEBIAN/md5sums"),
      payload
        .map(
          (f) =>
            `${createHash("md5").update(fs.readFileSync(f)).digest("hex")}  ${path.relative(dir, f)}`,
        )
        .join("\n") + "\n",
    );
    run("dpkg-deb", [
      ...
      "--build",
      dir,
      `/native/debs/${name}_${family.version}_${arch}.deb`,
    ]);
    evidence.push(provenance);
  }
fs.writeFileSync(
  "/native/evidence/packages.json",
