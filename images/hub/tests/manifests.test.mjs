...
test("Ubuntu acceptance uses native runners and cannot publish or bypass failed tests", async () => {
  const source = await read(
    "../../../.github/workflows/hub-ubuntu-acceptance.yml",
  );
  ...
  assert.ok(
    workflow.on.pull_request.paths.includes("images/hub/Dockerfile.ubuntu"),
  );
  assert.ok(workflow.on.pull_request.paths.includes("images/hub/src/**"));
  assert.ok(
    workflow.on.pull_request.paths.every(
      (p) => !p.endsWith(".md") && p !== "images/hub/**",
    ),
  );
  ...
  assert.ok(
    Object.values(job.env).every((value) => !String(value).includes("runner.")),
  );
  ...
  assert.ok(
    job.steps.some((s) =>
      s.run?.includes('test "$(uname -m)" = "$EXPECTED_MACHINE"'),
    ),
  );
  ...
  assert.ok(
    job.steps.some((s) =>
      s.run?.includes(
        "cc -O2 -Wall -Wextra /tests/native-library-regression.c",
      ),
    ),
  );
  assert.ok(
    job.steps.some((s) =>
      s.run?.includes("cc -O2 -Wall -Wextra /tests/elf-string-regression.c"),
    ),
  );
  assert.ok(
    job.steps.some((s) =>
      s.run?.includes(
        "-ec '/probes/native-library-regression && /probes/elf-string-regression'",
      ),
    ),
  );
  assert.ok(
    job.steps.some((s) =>
      s.run?.includes("--severity HIGH,CRITICAL --exit-code 1"),
    ),
  );
  assert.doesNotMatch(
    source,
    /setup-qemu|push:\s*true|docker push|login-action|secrets\.|continue-on-error|ignore-unfixed|detect_leaks=0|docker\.sock/,
  );
  for (const step of job.steps.filter((s) => s.uses))
    assert.match(step.uses, /@[a-f0-9]{40}$/);
  const artifact = job.steps.find((s) =>
...
test("Kubernetes example has one non-root replica, persistent data and no ingress", async () => {
  const documents = parseAllDocuments(
    await read("../examples/kubernetes/hub.yaml"),
  );
  ...
  assert.equal(
    values.some((v) => ["Service", "Ingress"].includes(v.kind)),
    false,
  );
});
test("ACA example uses environment-only policy, persistent data, singleton sizing and no ingress", async () => {
  ...
  assert.equal(
    template.volumes.find((v) => v.name === "data").storageType,
    "AzureFile",
  );
  assert.equal(
    template.volumes.some((v) => v.name === "config"),
    false,
  );
  const env = Object.fromEntries(
    template.containers[0].env.map((v) => [v.name, v.value]),
  );
...
test("PostgreSQL ACA example uses secret references, verified TLS and no durable volume", async () => {
  const {
    properties: { configuration, template },
  } = parse(await read("../examples/aca/containerapp.postgres.yaml"));
  assert.equal(configuration.activeRevisionsMode, "Single");
  assert.equal(configuration.ingress, undefined);
  assert.deepEqual(template.scale, { minReplicas: 1, maxReplicas: 1 });
  ...
  for (const name of ["HUB_PG_PASSWORD", "HUB_CREDENTIAL_KEY"]) {
    assert.ok(env[name].secretRef);
    assert.equal(env[name].value, undefined);
  }
  assert.equal(env.HUB_RUN_DIR.value, "/tmp/hub");
});
test("Hub publishing requires tests and native architectures; PR builds never login or push", async () => {
  ...
  const { test: tests, native } = workflow.jobs;
  assert.deepEqual(native.needs, ["changes", "test", "security"]);
  assert.equal(native.if, "needs.changes.outputs.hub == 'true'");
  assert.equal(native.uses, "./.github/workflows/native-image.yml");
  assert.equal(native.with.image, "devtunnel-toolkit-hub");
  assert.equal(
    native.with.publish,
    "${{ (github.event_name == 'push' && github.ref == 'refs/heads/main') || github.event_name == 'workflow_dispatch' }}",
  );
  assert.equal(tests.env, undefined);
  assert.ok(tests.steps.every((s) => !s.uses?.includes("login-action")));
  for (const command of ["npm run test:container", "npm run test:postgres"])
    assert.equal(
      tests.steps.find((s) => s.run === command).if,
  ...
  const shared = parse(
    await read("../../../.github/workflows/native-image.yml"),
  );
  for (const job of [
    shared.jobs.plan,
    shared.jobs.build,
    ...
    for (const step of job.steps.filter((s) =>
      ...
      const guard = [job.if, step.if].filter(Boolean).join(" && ");
      assert.match(guard, /inputs.publish/);
  ...
  assert.equal(build.provenance, true);
  assert.equal(build.sbom, true);
  assert.match(
    build.outputs,
    /push=\$\{\{ inputs.publish && github.event_name != 'pull_request'/,
  );
});
test("legacy image workflow uses a filtered matrix and skips an empty selection", async () => {
  ...
  assert.equal(
    ...
    "./.github/workflows/image-changes.yml",
  );
...
test("security gate scans changed images and never hides unpatched high or critical findings", async () => {
  ...
  const scan = workflow.jobs.images.steps.find((s) =>
    s.name?.startsWith("Block high"),
  ).run;
  ...
  assert.ok(
    workflow.jobs.source.steps.some(
      (s) => s.run?.includes("gitleaks") && s.run.includes("--redact"),
    ),
  );
...
test("release publishing requires the complete security workflow without suppressing functional checks", async () => {
  const checks = parse(
    await read("../../../.github/workflows/release-checks.yml"),
  );
  const suite = parse(
    await read("../../../.github/workflows/release-suite.yml"),
  );
  ...
  assert.equal(
    checks.jobs.security.with["scan-images"],
    "${{ inputs.container }}",
  );
  assert.equal(checks.jobs.checks.needs, undefined);
  assert.equal(
    suite.jobs.checks.uses,
    "./.github/workflows/release-checks.yml",
  );
  assert.ok(suite.jobs.build.needs.includes("checks"));
  assert.ok(suite.jobs.finalize.needs.includes("checks"));
  assert.ok(suite.jobs.record.needs.includes("build"));
  assert.ok(suite.jobs.finalize.needs.includes("record"));
