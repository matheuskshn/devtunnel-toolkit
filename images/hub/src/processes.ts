...
export async function waitFile(
  file: string,
  child: ChildProcess,
): Promise<void> {
  for (let i = 0; i < 100; i++) {
    if (child.exitCode !== null || child.signalCode !== null)
      throw new HubError("SESSION_SERVICE_FAILED");
    try {
      await access(file);
      return;
    } catch {
...
export async function waitSecretService(
  env: NodeJS.ProcessEnv,
  child: ChildProcess,
  signal: AbortSignal,
  options: { timeout?: number; query?: typeof command } = {},
): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    const reply = await (options.query ?? command)(
      "dbus-send",
      [
        "--session",
        "--print-reply",
    ...
    if (owned === undefined) {
      throw new HubError("SESSION_SERVICE_INVALID_REPLY");
    }
...
export async function command(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("error", () => {
      cleanup();
      reject(new HubError("COMMAND_UNAVAILABLE"));
    });
    child.once("close", (code) => {
      ...
      if (
        ...
        return reject(new HubError("AUTH_REQUIRED"));
      if (
        /^Tunnel not found(?: in [a-z0-9]+)?: [a-z0-9.-]+\s*$/im.test(
          stdout + stderr,
        )
      )
        return reject(new HubError("TUNNEL_NOT_FOUND"));
      if (/Conflict with existing entity/i.test(stdout + stderr))
        return reject(new HubError("TUNNEL_NAME_CONFLICT"));
