import { createServer as createSocketServer, createConnection, type Socket, type Server } from 'node:net';
import { createServer as createHttpServer, type Server as HttpServer } from 'node:http';
import { chmod, rm, rename } from 'node:fs/promises';
import { openSync, createReadStream, writeSync, constants, type ReadStream } from 'node:fs';
...
export class Manager {
  ...
  private maintenance = new Map<string, number>();
  ...
  async open(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => { this.socket!.once('error', reject); this.socket!.listen(address, resolve); });
    await chmod(address, 0o600);
    this.health = createHttpServer((req, res) => {
      const found = ['/live', '/ready'].includes(req.url ?? '');
      const ok = req.url === '/live' ? !this.stopping : this.healthy && !this.stopping;
      res.writeHead(found ? (ok ? 200 : 503) : 404, { 'Content-Type': 'application/json' });
  ...
  private configureSquid(): Promise<void> {
    const task = this.configQueue.then(async () => {
      ...
      await atomicWrite(candidate, squidConfig(this.config, this.store.state.sessions, this.runDir));
      await command('squid', ['-k', 'parse', '-f', candidate], cleanEnvironment());
      this.healthy = false;
      await rename(candidate, current);
      if (this.squid) {
        await command('squid', ['-k', 'reconfigure', '-f', current], cleanEnvironment());
      } else {
        ...
        this.squid.once('exit', () => {
          this.healthy = false;
          if (!this.stopping) { this.log('squid_failed'); void this.close().finally(() => process.exit(1)); }
        });
      }
    ...
    this.configQueue = task.catch(() => {}); return task;
  }
  private async runtime(s: Session): Promise<SessionRuntime> {
    ...
    if (!runtime) {
      ...
      runtime = new SessionRuntime(s, this.dataDir, directory, () => {
        if (!this.stopping) {
  ...
  private async provision(s: Session, runtime: SessionRuntime): Promise<void> {
    const create = async () => {
      ...
      s.tunnel_id = id; await this.store.save();
    };
    if (!s.tunnel_id) await create();
    let tunnel;
    try { tunnel = await runtime.details(); }
    catch (e) {
  ...
  private async start(s: Session): Promise<void> {
    ...
    check(s.identity, 'AUTH_REQUIRED'); check(!this.workers.has(s.id), 'SESSION_ALREADY_RUNNING');
    ...
    await this.configureSquid();
    s.desired = true; s.status = 'starting'; delete s.error; await this.store.save();
    const worker = fork(new URL('./worker.js', import.meta.url), [], {
      env: runtime.env, stdio: ['ignore', 'ignore', 'ignore', 'ipc'],
    });
    this.workers.set(s.id, worker);
    worker.once('error', () => { void this.fail(s, new HubError('WORKER_FAILED')); });
    worker.on('message', (message: any) => {
      if (message.type === 'credentials') {
        ...
        s.status = message.type === 'ready' || message.status === 'connected' ? 'running' : 'starting';
        if (s.status === 'running') this.failures.delete(s.id);
        void this.store.save().catch(() => { void this.close(); });
      }
    });
  ...
  private async credentialsForWorker(s: Session, runtime: SessionRuntime, worker: ChildProcess, id: number): Promise<void> {
    ...
    try {
      ...
      if (Date.now() - (this.maintenance.get(s.id) ?? 0) > 12 * 3600000) {
        await runtime.cli(['update', s.tunnel_id!, '--expiration', '2d', '--json']);
        this.maintenance.set(s.id, Date.now());
      }
      if (worker.connected) worker.send({ type: 'credentials', id, tunnel }, () => {});
    } catch (e) {
      if (this.workers.get(s.id) !== worker) return;
      if (worker.connected) worker.send({ type: 'credentials', id, error: errorCode(e) }, () => {});
      await this.fail(s, e);
    }
  }
  private async fail(s: Session, e: unknown): Promise<void> {
    ...
    s.status = code === 'AUTH_REQUIRED' ? 'reauth_required' : 'error'; s.error = code;
    this.log('session_failed', s, code); await this.store.save();
    ...
    if (s.desired && ['WORKER_FAILED', 'COMMAND_FAILED', 'COMMAND_CANCELLED_OR_TIMEOUT', 'TUNNEL_NOT_FOUND'].includes(code)) {
      const attempts = (this.failures.get(s.id) ?? 0) + 1; this.failures.set(s.id, attempts);
      if (attempts <= 8) this.retries.set(s.id, setTimeout(() => {
        this.track(this.exclusive(s.id, () => this.start(s)).catch(error => this.fail(s, error)));
      }, Math.min(300000, 1000 * 2 ** attempts)));
    }
  }
  private async stopWorker(s: Session): Promise<void> {
    clearTimeout(this.retries.get(s.id)); this.retries.delete(s.id);
  ...
  async dispatch(args: string[], output: (chunk: string) => void): Promise<unknown> {
    ...
    if (operation === 'status') { check(args.length === 3, 'USAGE'); return this.store.get(id); }
    return this.exclusive(id, async () => {
      if (operation === 'add') {
        check(flags.length % 2 === 0, 'USAGE');
        const options = new Map<string, string>();
        for (let i = 0; i < flags.length; i += 2) {
          check(['--provider', '--tunnel-name'].includes(flags[i]) && !options.has(flags[i]), 'USAGE'); options.set(flags[i], flags[i+1]);
        }
        const s = this.store.add(id, (options.get('--provider') ?? 'microsoft') as Provider, options.get('--tunnel-name'));
        await this.store.save(); return s;
      }
      check(flags.length === 0 && ['login','start','stop','logout','remove'].includes(operation), 'USAGE');
      const s = this.store.get(id);
      try {
        if (operation === 'start') { await this.start(s); return s; }
        if (operation === 'stop') {
          s.desired = false; await this.stopWorker(s); s.status = s.identity ? 'stopped' : 'login_required';
          ...
          s.desired = false; await this.stopWorker(s);
          const runtime = await this.runtime(s);
          await runtime.cli(['user','logout']); await runtime.close(); this.runtimes.delete(s.id);
        ...
        await this.store.save(); this.log(`session_${operation}`, s); return s;
      } catch (e) { await this.fail(s, e); throw e; }
    });
  }
  private accept(socket: Socket): void {
    this.clients.add(socket); socket.on('close', () => this.clients.delete(socket)); socket.on('error', () => {});
    socket.setTimeout(650000, () => socket.destroy());
    let buffer = ''; let received = false;
    socket.on('data', chunk => {
      ...
      this.track((async () => {
        try {
          ...
          check(Array.isArray(args) && args.every(x => typeof x === 'string'), 'INVALID_REQUEST');
          const result = await this.dispatch(args, chunk => {
            // Device code is delivered ONLY to the connected administrator socket.
            if (!socket.destroyed) socket.write(JSON.stringify({ output: chunk }) + '\n');
          });
  ...
  async close(): Promise<void> {
    ...
    await Promise.all([...this.workers.values()].map(w => terminate(w))); this.workers.clear();
