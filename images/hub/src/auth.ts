...
export function parseJson(text: string): Record<string, any> {
  ...
  try {
    ...
    throw new HubError("CLI_JSON_INVALID");
  }
}
export function parseIdentity(
  ...
  check(
    ["microsoft", "entra", "github"].includes(provider),
  ...
  check(
    typeof login === "string" &&
      login.length > 0 &&
      login.length <= 320 &&
      !/[\x00-\x1f\x7f]/.test(login),
    "IDENTITY_SCHEMA_UNSUPPORTED",
  );
  check(
    (typeof id === "string" || typeof id === "number") &&
      String(id).length > 0 &&
      String(id).length <= 256,
    "IDENTITY_SCHEMA_UNSUPPORTED",
  );
  ...
  return {
    provider: expected,
    user_login: login,
    user_id: `${expected}:${tenantPrefix}${id}`,
...
export function canonicalTunnel(value: Record<string, any>): string {
  ...
  check(validCanonicalId(canonical), "TUNNEL_SCHEMA_UNSUPPORTED");
  return canonical;
}
export function privateTunnel(
  value: Record<string, any>,
  session: Session,
  ports: number | readonly number[] = [
    session.proxy_port ?? DEFAULT_PROXY_PORT,
    ...(session.socks_port === undefined ? [] : [session.socks_port]),
  ],
): Tunnel {
  const tunnel = value.tunnel ?? value;
  ...
  check(
    tunnel.accessControl && Array.isArray(tunnel.accessControl.entries),
  ...
  check(
    new Set(expected).size === expected.length &&
      ...
      tunnel.ports.length === expected.length &&
      new Set(tunnel.ports.map((p: any) => p?.portNumber)).size ===
        expected.length &&
      tunnel.ports.every((p: any) => expected.includes(p?.portNumber)),
    "TUNNEL_PORT_POLICY_CHANGED",
  );
  for (const port of tunnel.ports) {
    check(
      port.accessControl === undefined ||
        (port.accessControl !== null &&
          typeof port.accessControl === "object" &&
          !Array.isArray(port.accessControl) &&
          Array.isArray(port.accessControl.entries) &&
          port.accessControl.entries.length === 0),
      "PORT_NOT_OWNER_ONLY",
    );
  }
...
export class SessionRuntime {
  ...
  private async openServices(): Promise<void> {
    check(
      !this.closing && !this.abort.signal.aborted,
    ...
    this.keyring = launch(
      ...
      [
        ...
        "--unlock",
        "--components=secrets",
        `--control-directory=${this.env.GNOME_KEYRING_CONTROL}`,
      ],
      this.env,
    );
    this.keyring.stdout?.resume();
    this.keyring.stdin?.end("\n");
  ...
  cli(args: string[], output?: (chunk: string) => void): Promise<string> {
    return this.withCredentials(() =>
      command("devtunnel", args, this.env, {
        timeout: output ? 600000 : 60000,
        output,
  ...
  withCredentials<T>(action: () => Promise<T>): Promise<T> {
    const task = this.queue.then(async () => {
      ...
      if (this.persistence) {
        ...
        await this.openServices().catch(async (e) => {
  ...
  async identity(): Promise<Identity> {
    const identity = parseIdentity(
      parseJson(await this.cli(["user", "show", "--json"])),
      this.session.provider,
    );
    check(
      identity.provider !== "microsoft" ||
        !this.allowedTenants.length ||
        (identity.tenant_id &&
  ...
  async login(output: (chunk: string) => void): Promise<void> {
    ...
    await this.cli(
      [
        "user",
        "login",
        this.session.provider === "github" ? "--github" : "--entra",
    ...
    try {
      bindIdentity(this.session, identity);
    } catch (e) {
  ...
  async details(): Promise<Tunnel> {
    ...
    try {
      ...
      const tunnel = await client.getTunnel(
        { tunnelId, clusterId, accessTokens: { host: token } },
      ...
      if (status === 401) throw new HubError("AUTH_REQUIRED");
      throw new HubError(
        status === 403 ? "TUNNEL_ACCESS_DENIED" : "COMMAND_FAILED",
      );
    } finally {
      clearTimeout(timer);
  ...
  async close(): Promise<void> {
    ...
    await this.stopServices();
    if (this.persistence)
      await rm(this.env.HOME!, { recursive: true, force: true });
  }
  private async stopServices(): Promise<void> {
    this.cycling = true;
    await terminate(this.keyring);
    await terminate(this.bus);
