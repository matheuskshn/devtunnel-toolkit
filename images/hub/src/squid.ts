...
export function squidConfig(
  config: Config,
  sessions: Session[],
  runDir: string,
): string {
  if (!/^\/[a-zA-Z0-9/_-]+$/.test(runDir))
    throw new Error("Invalid runtime path");
  const active = sessions.filter((s) => s.identity && s.status !== "removed");
  const lines = [
    "visible_hostname devtunnel-toolkit-hub",
    "workers 1",
    `pid_filename ${runDir}/squid.pid`,
    "cache_effective_user hub",
    "cache_effective_group hub",
    "cache deny all",
    "cache_mem 16 MB",
    ...
    "acl metadata dst 169.254.0.0/16 fe80::/10",
    "http_access deny metadata",
    "acl loopback_destination dst 0.0.0.0/8 127.0.0.0/8 ::/128 ::1/128",
    "http_access deny loopback_destination",
  ];
  if (active.length) {
    for (const s of active)
      lines.push(`http_port 127.0.0.1:${s.listener} name=s${s.listener}`);
    const listenerNames = active.map((s) => "s" + s.listener).join(" ");
    lines.push(`acl session_listener myportname ${listenerNames}`);
    if (config.allowAllDomains) {
      ...
      lines.push(
        `acl allowed_destinations dstdomain ${config.allowedDomains.join(" ")}`,
        "http_access allow session_listener allowed_destinations",
      );
    }
  } else {
...
export function auditRecord(
  ...
  const [time, listener, method, destination, port, status, bytes, duration] =
    fields;
  const session = sessions.find((s) => s.listener === Number(listener));
  if (!session?.identity) return;
  const decode = (s: string) => {
    try {
      return decodeURIComponent(s).slice(0, 253);
    } catch {
      return "invalid";
    }
  };
  return {
    ...
    ...session.identity,
    tunnel_id: session.tunnel_id ?? null,
    attribution: "session_listener",
    listener: Number(listener),
    destination: decode(destination),
    destination_port: Number(port) || null,
