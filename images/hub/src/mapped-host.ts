...
export { DEFAULT_PROXY_PORT as PUBLISHED_PROXY_PORT } from "./model.js";
export class MappedTunnelHost extends TunnelRelayTunnelHost {
  private disposed?: Promise<void>;
  private readonly mappings: ReadonlyMap<number, number>;
  constructor(
    management: TunnelManagementClient,
    readonly listenerPort: number,
    readonly proxyPort = DEFAULT_PROXY_PORT,
    ...
    if (
      socks &&
      (!Number.isInteger(socks.proxyPort) ||
        !Number.isInteger(socks.listenerPort) ||
        socks.proxyPort < 1024 ||
        socks.proxyPort > 65535 ||
        socks.listenerPort < 1024 ||
        socks.listenerPort > 65535 ||
        new Set([proxyPort, listenerPort, socks.proxyPort, socks.listenerPort])
          .size !== 4)
    )
      throw new TypeError("Invalid SOCKS mapping");
    super(management, () => {}); // SDK traces may contain credentials or remote details.
    this.mappings = new Map([
      [proxyPort, listenerPort],
      ...(socks
        ? [[socks.proxyPort, socks.listenerPort] as [number, number]]
        : []),
    ]);
  }
  ...
  override async refreshPorts(): Promise<void> {
    ...
    if (
      this.tunnel?.ports?.length !== this.mappings.size ||
      new Set(this.tunnel.ports.map((p) => p.portNumber)).size !==
        this.mappings.size ||
      this.tunnel.ports.some((p) => !this.mappings.has(p.portNumber))
    ) {
      await this.dispose();
      throw new Error("TUNNEL_PORT_POLICY_CHANGED");
    }
  ...
  override async forwardPort(
    ...
    if (listener === undefined)
      throw new Error("Host only publishes configured proxy ports");
    pfs.acceptRemoteConnectionsForNonForwardedPorts = false;
    const existing = pfs.localForwardedPorts.find(
      (p) => p.remotePort === port.portNumber,
    );
    if (existing) {
      if (existing.localPort !== listener)
        throw new Error("A different listener is registered");
      return;
    }
    ...
    this.remoteForwarders.set(
      new SessionPortKey(pfs.session.sessionId, port.portNumber).toString(),
      forwarder,
    );
  }
}
