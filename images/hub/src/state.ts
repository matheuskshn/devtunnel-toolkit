...
import { check, validId, validTunnelName, validCanonicalId, validNameTemplate, resolveTunnelName, type Config, type State, type Session, type Provider } from './model.js';
...
export class StateStore {
  ...
  async load(): Promise<void> {
    ...
    for (const session of s.sessions) {
      check(validId(session.id) && ['microsoft', 'github'].includes(session.provider), 'INVALID_STATE');
      const named = validTunnelName(session.tunnel_name);
      check(named ? session.tunnel_name_template === undefined :
        session.tunnel_name === undefined && validNameTemplate(session.tunnel_name_template) &&
        session.tunnel_id === undefined && !session.desired && ['login_required','error','reauth_required','stopped','removed'].includes(session.status), 'INVALID_STATE');
      ...
      check(!ids.has(session.id) && !listeners.has(session.listener) && (!session.tunnel_name || !names.has(session.tunnel_name)), 'DUPLICATE_STATE_MAPPING');
      ...
      ids.add(session.id); listeners.add(session.listener); if (session.tunnel_name) names.add(session.tunnel_name);
    }
  ...
  add(id: string, provider: Provider, name?: string): Session {
    ...
    check(!this.state.sessions.some(s => s.id === id || (name !== undefined && s.tunnel_name === name)), 'SESSION_OR_NAME_RESERVED');
    ...
    const session: Session = { id, provider, ...(name === undefined ? { tunnel_name_template: this.config.tunnelNameTemplate } : { tunnel_name: name }), listener: this.state.next_listener++,
  ...
  resolveName(session: Session): void {
    ...
    if (session.tunnel_name !== undefined) return;
    check(session.identity && session.identity.provider === session.provider, 'AUTH_REQUIRED');
    check(session.tunnel_name_template, 'INVALID_TUNNEL_NAME_TEMPLATE');
    const name = resolveTunnelName(session.tunnel_name_template, this.config.hubId, session.identity);
    check(!this.state.sessions.some(s => s !== session && s.tunnel_name === name), 'SESSION_OR_NAME_RESERVED');
