...
const run = (file, args) =>
  execFileSync(trustedExecutable(file), args, {
    encoding: "utf8",
...
export async function remoteTag(api, tag) {
  ...
  for (let depth = 0; depth < 5; depth++) {
    ...
    if (ref.object.type !== "tag") throw new Error("INVALID_TAG_TARGET");
    ref = await api(`git/tags/${ref.object.sha}`);
  }
  throw new Error("TAG_DEPTH_LIMIT");
}
function existingDraft(existing, info, sha) {
  const matches = existing.filter((r) => r.tag_name === info.tag);
  if (matches.length > 1) throw new Error("DUPLICATE_RELEASE");
  if (matches[0] && !matches[0].draft)
    throw new Error("RELEASE_ALREADY_PUBLISHED");
  if (
    /^[a-f0-9]{40}$/.test(matches[0]?.target_commitish ?? "") &&
    matches[0].target_commitish !== sha
  )
    throw new Error("RELEASE_DRAFT_COMMIT_MISMATCH");
  return matches[0];
}
function validateReleaseOrder(existing, info) {
  if (info.prerelease) return;
  const core = (v) => [v.major, v.minor, v.patch].map(BigInt);
  for (const release of existing.filter((r) => !r.draft && !r.prerelease)) {
    let previous;
    try {
      previous = versionInfo(release.tag_name.slice(1));
    } catch {
      continue;
    }
    ...
    if (different >= 0 && a[different] > b[different])
      throw new Error("RELEASE_OUT_OF_ORDER");
  }
}
export async function prepareRelease({ api = github, version, sha, notes }) {
  ...
  if (currentTag && currentTag !== sha) throw new Error("RELEASE_TAG_MOVED");
  const existing = await releases(api);
  const draft = existingDraft(existing, info, sha);
  validateReleaseOrder(existing, info);
  if (!currentTag) {
    const annotated = await api("git/tags", {
      method: "POST",
      body: {
        tag: info.tag,
        message: `Release ${info.tag}`,
        object: sha,
        type: "commit",
      },
    });
    await api("git/refs", {
      method: "POST",
      body: { ref: `refs/tags/${info.tag}`, sha: annotated.sha },
    });
  }
  return (
    draft ??
...
export function registryDigest(ref) {
  try {
    return `sha256:${createHash("sha256")
      .update(skopeo("inspect", ["--raw", `docker://${ref}`]))
      .digest("hex")}`;
  } catch (e) {
    ...
    if (
      /manifest unknown|name unknown|manifest_unknown|name_unknown/i.test(
        String(e.stderr),
      )
    )
      return undefined;
...
function repositories(image) {
  ...
  if (process.env.DOCKERHUB_USERNAME && process.env.DOCKERHUB_TOKEN) {
    ...
    if (!/^[a-z0-9][a-z0-9_-]*$/.test(namespace))
      throw new Error("INVALID_DOCKERHUB_NAMESPACE");
...
export async function promoteSuite({
  ...
  await verifyTag();
  const immutable = records.flatMap((record) =>
    repoFor(record.image).flatMap((repository) =>
      [version, `v${version}`].map((tag) => ({
        source: `${repoFor(record.image)[0]}@${record.digest}`,
  ...
  for (const item of immutable)
    immutableTag(await inspect(item.target), item.digest);
  for (const item of immutable) {
    if (immutableTag(await inspect(item.target), item.digest))
      await copy(item.source, item.target);
    if ((await inspect(item.target)) !== item.digest)
      throw new Error("PUBLISHED_DIGEST_MISMATCH");
  }
  await attach();
  // Mutable aliases only move after every immutable tag and release asset is verified.
  await promoteAliases({ records, repoFor, version, copy, inspect });
  await verifyTag();
  await publish();
}
async function promoteAliases({ records, repoFor, version, copy, inspect }) {
  for (const record of records)
    for (const repository of repoFor(record.image))
      for (const alias of releaseAliases(version)) {
        const target = `${repository}:${alias}`;
        await copy(`${repoFor(record.image)[0]}@${record.digest}`, target);
        if ((await inspect(target)) !== record.digest)
          throw new Error("ALIAS_DIGEST_MISMATCH");
      }
}
...
async function uploadAssets(release, files) {
  ...
  for (const file of files) {
    ...
    if (previous) {
      if (previous.digest !== checksum)
        throw new Error("RELEASE_ASSET_CONFLICT");
      continue;
    }
    ...
    if (asset.digest !== checksum)
      throw new Error("RELEASE_ASSET_DIGEST_MISMATCH");
  }
}
function recordCandidate(mode, info, sha) {
  const image = process.env.RELEASE_IMAGE;
  if (!suiteImages.some((i) => i.image === image))
    throw new Error("UNKNOWN_RELEASE_IMAGE");
  const built = candidate(image, info.version, sha);
  if (mode === "candidate") {
    output("exists", !!built.digest);
    output("ref", built.ref);
    return;
  }
  if (!built.digest) throw new Error("CANDIDATE_MISSING");
  ...
  for (const [key, property] of [
    ...
    const data = JSON.parse(
      run("docker", [
        ...
        "imagetools",
        "inspect",
        `${built.repository}@${built.digest}`,
        "--format",
        `{{json .${property}}}`,
      ]),
    );
    writeFileSync(
      `release-assets/${image}.${key}.json`,
...
async function finalizeRelease(info, sha) {
  ...
  const records = suiteImages.map((i) =>
    JSON.parse(readFileSync(`release-assets/${i.image}.json`, "utf8")),
  );
  const verified = validateManifest(records, info.version, sha).sort((a, b) =>
    a.image.localeCompare(b.image),
  );
  for (const record of verified)
    for (const kind of ["sbom", "provenance"]) {
      const data = JSON.parse(
        readFileSync(`release-assets/${record.image}.${kind}.json`, "utf8"),
      );
    ...
    JSON.stringify(
      {
        version: info.version,
        sha,
        images: verified.map((r) => ({
          ...r,
          repositories: repositories(r.image),
        })),
      },
      null,
      2,
  ...
  const files = readdirSync("release-assets")
    ...
    .sort()
    .map((f) => `release-assets/${f}`);
  writeFileSync(
    "release-assets/SHA256SUMS",
    files
      .map(
        (f) =>
          `${createHash("sha256").update(readFileSync(f)).digest("hex")}  ${path.basename(f)}\n`,
      )
      .join(""),
  );
  const imageNotes = verified
    .flatMap((record) =>
      repositories(record.image).map(
        (repository) =>
          `- \`${repository}:${info.version}\` - \`${record.digest}\``,
      ),
    )
    .join("\n");
  await promoteSuite({
    ...
    verifyTag: async () => {
      if ((await remoteTag(github, info.tag)) !== sha)
        throw new Error("RELEASE_TAG_MOVED");
    },
    attach: () =>
      uploadAssets(release, [...files, "release-assets/SHA256SUMS"]),
    publish: () =>
      github(`releases/${release.id}`, {
        method: "PATCH",
        body: {
          draft: false,
          prerelease: info.prerelease,
          make_latest: info.prerelease ? "false" : "true",
...
async function main() {
  const mode = process.argv[2],
    info = checkVersions(process.cwd(), process.env.RELEASE_TAG);
  ...
  if (mode === "prepare") {
    ...
    output("sha", sha);
    output("matrix", { include: suiteImages });
    return;
  }
  if (mode === "candidate" || mode === "record")
    return recordCandidate(mode, info, sha);
  if (mode === "finalize") return finalizeRelease(info, sha);
